...
public:
	...
	float UpdateDepthTexture()
	{
		...
		Image* img;
		if (activeWall>-1 && activeWall<Walls.size())
		...
		pixelBuffer->unlock();
		if (userPix)
			return (float)outPix/(float)userPix;
		else
			return -1;
	...
	XnStatus initPrimeSensor()
	{
		...
		m_candidateID = 0;
		m_pStartPoseDetector = new StartPoseDetector(2.0);
		m_pEndPoseDetector = new EndPoseDetector(m_UserGenerator, 5.0);
		m_pEndPoseDetector->SetUserId(m_candidateID);

		return XN_STATUS_OK;
	}

	float addTime(Real deltaTime)
	{
		try{
		...
		float outPerc=UpdateDepthTexture();
...
private:
	...
	{
		// create main model
		mBodyNode = sceneMgr->getRootSceneNode()->createChildSceneNode(Vector3::UNIT_Y * 2*CHAR_HEIGHT);
		...
		mBodyNode->scale(2,2,2);
		mBodyNode->translate(0,0,3);
	...
	void setupCamera(Camera* cam)
	{
		...
		cam->setPosition(50,-10,00);
		mCameraGoal = mCameraPivot->createChildSceneNode(Vector3(0, 20, 0));
	...
	void PSupdateBody(Real deltaTime)
	{
		...
		if (pUserSkel.IsTracking(m_candidateID))
		{
			...
			{			 
				...
				if(torsoPos.fConfidence > 0.5)
				{
					rootBone->setPosition(newPos2/3);
				}
			}
	...
	void updateBody(Real deltaTime)
	{
		...
		if (mKeyDirection != Vector3::ZERO && mBaseAnimID != ANIM_DANCE)
		{
			...
			//mCameraPivot->translate(0, 0, deltaTime * RUN_SPEED * mAnims[mBaseAnimID]->getWeight(),Node::TS_LOCAL);
		...
		if (mBaseAnimID == ANIM_JUMP_LOOP)
		{
			...
			mBodyNode->translate(0, mVerticalVelocity * deltaTime/3, 0, Node::TS_LOCAL);
	...
	void updateCamera(Real deltaTime)
	{
		...
		////mCameraPivot->translate(mKeyDirection.x*deltaTime*10,0,mKeyDirection.z*deltaTime*10);
